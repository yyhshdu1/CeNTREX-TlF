...
def H_LD(psi):
    ...
    for Pprime in [-1,1]:
        ...
        ket = CoupledBasisState(F, mF, F1, J, I1, I2, Omega = psi.Omega, P = Pprime,
...
def H_mhf_Tl(psi):
    ...
    for Jprime in np.arange(np.abs(J-1),J+2):
        ...
        if np.abs(Fprime-Jprime) <= (I1+I2):
            ...
            basis_state = CoupledBasisState(Fprime, mFprime, F1prime, Jprime, 
                                    I1prime, I2prime, Omega = psi.Omega, P = P,
                                    electronic_state = psi.electronic_state
                                    )
...
def H_mhf_F(psi):
    ...
    for Jprime in np.arange(np.abs(J-1), J+2):
        ...
        for F1prime in np.arange(np.abs(Jprime-I1), Jprime+I1+1):
            ...
            basis_state = CoupledBasisState(Fprime, mFprime, F1prime, Jprime, 
                                    I1prime, I2prime, P = P, Omega = psi.Omega,
                                    electronic_state=psi.electronic_state
                                    )
...
def H_c_Tl(psi):
    ...
    basis_state = CoupledBasisState(Fprime, mFprime, F1prime, Jprime, I1prime, 
                                    I2prime, Omega = psi.Omega, P = psi.P,
                                    electronic_state=psi.electronic_state)
...
def H_cp1_Tl(psi):
    ...
    for Jprime in range(np.abs(J-1), J+2):
        for Pprime in [-1,1]:
            amp = ((P*(-1)**(J-S) * ME(J,Jprime,1,-1) + Pprime*(-1)**(Jprime-S) 
                    * ME(J,Jprime,-1,1))
                    *(-1)**float((J-Jprime) !=0)/2)
                   
                   
            ket = CoupledBasisState(Fprime, mFprime, F1prime, Jprime, I1prime, 
                                    I2prime,  Omega = psi.Omega, P = Pprime,
                                    electronic_state = psi.electronic_state)
