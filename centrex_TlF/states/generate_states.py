...
def generate_coupled_states_base(qn_selector: QuantumSelector) -> np.ndarray:
    ...
    assert qn_selector.J is not None, 'function requires a rotational quantum number to be set'
    assert qn_selector.electronic is not None, 'function requires electronic state to be set'
    ...
    for par in ['J', 'F1', 'F', 'mF', 'electronic', 'P', 'Ω']:
        ...
        quantum_numbers.append([par] if not isinstance(par, (list, tuple, np.ndarray)) else par)
                ...
                for F in Fsl:
                    ...
                    for mF in mFsl:
                        ...
                        for P in Ps:
                            ...
                            for Ω in Ωs:
                                QN.append(CoupledBasisState(F,mF,F1,J,I_F,I_Tl, 
                                        electronic_state=estate, P = P, Ω=Ω))
    return np.asarray(QN)

def generate_coupled_states_ground_X(qn_selector: Union[QuantumSelector, list, np.ndarray]) -> np.ndarray:
...
def generate_coupled_states_excited_B(qn_selector: Union[QuantumSelector, list, np.ndarray]) -> np.ndarray:
